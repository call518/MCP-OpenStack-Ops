...
[[ -d src/mcp_openstack_ops || -d src/$PKG_NAME ]] || die "src/mcp_openstack_ops not found"
if [[ -d src/mcp_openstack_ops && ! -d src/$PKG_NAME ]]; then mv src/mcp_openstack_ops "src/$PKG_NAME"; fi
...
for f in $FILES; do
  ...
  grep -qE 'mcp_openstack_ops|mcp-openstack-ops|mcp-openstack-ops' "$f" 2>/dev/null || continue
  sed -i -e "s/mcp_openstack_ops/${PKG_NAME}/g" -e "s/mcp-openstack-ops/${DIST_NAME}/g" -e "s/mcp-openstack-ops/${DIST_NAME}/g" "$f" || true
done
